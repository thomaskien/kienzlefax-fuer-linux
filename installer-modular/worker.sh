sep "Install Worker (kienzlefax-worker.py v1.3.6) – FULL CONTENT"
...
python3 -m py_compile "$WORKER_PATH"

sep "Install AGI (kfx_update_status.agi v1.3.6) – FULL CONTENT"
backup_file "$AGI_PATH"
mkdir -p "$(dirname "$AGI_PATH")"
cat >"$AGI_PATH" <<'PY'
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
def decide_from_send_end(dialstatus: str, hangupcause: str, faxstatus: str, faxerror: str, pages_sent: Optional[int]) -> Tuple[str, str]:
...
def decide_from_dial_end(dialstatus: str) -> Tuple[Optional[str], Optional[str]]:
    if dialstatus == "ANSWER":
        return None, None
    if dialstatus in ("BUSY", "NOANSWER", "CONGESTION", "CHANUNAVAIL"):
        return "RETRY", dialstatus
...
def main() -> int:
    ...
    eprint("kfx_update_status.agi: unknown action, ignoring")
    return 0
