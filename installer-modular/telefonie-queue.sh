...
PY
  ...
  for capacity_value in "$PROVIDER_CHANNEL_LIMIT" "$PROVIDER_PHONE_LIMIT" "$PROVIDER_FAX_LIMIT" "$QUEUE_MAX_WAITING" "$SIPGATE_OVERFLOW_CHANNEL_LIMIT"; do
...
if [[ "$PHONE_ENABLED" == "y" ]]; then
  ...
  if [[ "$SIPGATE_OVERFLOW_ENABLED" == "y" ]]; then
    echo "[INFO] Sipgate-Ueberlauf: aktiv, separate additive Grenze ${SIPGATE_OVERFLOW_CHANNEL_LIMIT}."
  fi
