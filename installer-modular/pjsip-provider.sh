...
if [[ "${KFX_QUEUE_ONLY:-n}" == "y" ]]; then
  ...
  LOCAL_NET="${KFX_PHONE_LOCAL_CIDR:-}"
  PJSIP="/etc/asterisk/pjsip.conf"
  [[ "$SIP_BIND_PORT" =~ ^[0-9]+$ ]] || { echo "ERROR: KFX_SIP_BIND_PORT ungueltig." >&2; exit 1; }

  TMP="$(mktemp)"
  trap 'rm -f "$TMP"' EXIT
  {
    cat <<EOF
; generated by kienzlefax provider module: queue-only, no fax endpoint
...
EOF
  ...
  if [[ ! -f "$PJSIP" ]] || ! cmp -s "$TMP" "$PJSIP"; then
    ...
    (( ACTIVE_CHANNELS == 0 )) || {
      echo "ERROR: Queue-only-PJSIP-Aenderung braucht einen Neustart, aber ${ACTIVE_CHANNELS} Kanaele sind aktiv." >&2
      exit 1
    }
