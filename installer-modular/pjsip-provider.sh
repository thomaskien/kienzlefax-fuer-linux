...
active_channel_count() {
  local out
  out="$(asterisk -rx "core show channels concise" 2>/dev/null || true)"
...
if command -v asterisk >/dev/null 2>&1; then
  if [[ "$PJSIP_RESTART_REQUIRED" == "y" ]]; then
    ...
    if (( ACTIVE_CHANNELS > 0 )); then
      echo "ERROR: PJSIP-System-/Transportaenderung braucht einen Neustart, aber ${ACTIVE_CHANNELS} Kanaele sind aktiv." >&2
      exit 1
    fi
    echo "[INFO] Starte Asterisk fuer PJSIP-System-/Transportaenderung kontrolliert neu..."
    systemctl restart asterisk || service asterisk restart
    wait_for_asterisk_cli 90 || { echo "ERROR: Asterisk CLI nach Restart nicht bereit." >&2; exit 1; }
  else
